...
const getAllOrders = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const allOrders = (created_at === 'today'
    ...
    const orders = (created_at === 'today'
        ? yield (0, db_1.query)("SELECT * FROM orders WHERE created_at >= now()::date - interval '0h' ORDER BY created_at DESC OFFSET $1 LIMIT $2;", [pageSize * (page - 1), pageSize])
        : created_at === 'week'
            ? yield (0, db_1.query)("SELECT * FROM orders WHERE created_at >= now()::date - interval '7d' ORDER BY created_at DESC OFFSET $1 LIMIT $2;", [pageSize * (page - 1), pageSize])
            : created_at === 'month'
                ? yield (0, db_1.query)("SELECT * FROM orders WHERE created_at >= now()::date - interval '30d' ORDER BY created_at DESC OFFSET $1 LIMIT $2;", [pageSize * (page - 1), pageSize])
                : yield (0, db_1.query)("SELECT * FROM orders WHERE created_at >= now()::date - interval '365d' ORDER BY created_at DESC OFFSET $1 LIMIT $2;", [pageSize * (page - 1), pageSize])).rows;
    // for (const order of orders) {
    //   const fullProducts = (
    //     await query(
    //       'SELECT * FROM products INNER JOIN (SELECT * FROM order_product WHERE order_id = $1) order_product ON products.id = order_product.product_id;',
